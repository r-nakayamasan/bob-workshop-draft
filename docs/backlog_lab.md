...
!!! warning "はじめにお読みください"
    - **生成 AI の出力にはブレがあります。** 同じプロンプトを実行しても、毎回まったく同じ出力になるとは限りません。ハンズオン内の「期待する出力例」はあくまで参考です。
    - **本番プロジェクトは使わないでください。** Backlog MCP は課題の読み取りだけでなく、コメント投稿や状態変更など**書き込み操作**も行います。必ず検証用スペース・プロジェクトで実施してください。
    - **Backlog に書き込む前に内容を確認してください。** プロンプトに「まだ更新しないでください」と明記することで、Bob が下書きを表示してから確認のうえ実行できます。

!!! info "Backlog MCP の今後"
    現在 Backlog MCP は個別に設定が必要ですが、今後は **Bob の MCP マーケットプレイス**からワンクリックで導入できる形式での提供も計画されています。
...
1. Bob のチャットパネル右上にある **歯車アイコン（⚙）** をクリックします
2. 設定パネルが開いたら、左側のタブから **「MCP」** を選択します
3. 画面内の **「＋」ボタン** をクリックして MCP サーバーを追加します

### Bob に Backlog MCP を設定する

MCP 設定画面が開いたら、以下の JSON を入力して追加します。
